...
export default async function recipes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: RecipeCreateMultipartBody }>(
    '/recipes',
    { schema: createRecipeMultipartSchema, preHandler: [ fastify.verifyBearerAuth ] },
    async (req, reply) => {
      try {
        let recipe
        if (req.isMultipart()) {
  ...
  }>('/recipes/:id', { schema: updateRecipeMultipartSchema, preHandler: [ fastify.verifyBearerAuth ] }, async (req) => {
    try {
      if (req.isMultipart()) {
  ...
  fastify.delete<{
    Params: RecipeParams
  }>('/recipes/:id', { schema: deleteRecipeSchema, preHandler: [ fastify.verifyBearerAuth ] }, async (req, reply) => {
    try {
